...
class UnifiedCompleter(Completer):
    ...
    def update_resources(self, resources: List):
        ...
        # print resource IDs for debugging
        print(Fore.CYAN + f"\n type of resources:", type(self.resources))
        print(Fore.CYAN + f"\n Resources:", self.resources)
        print(Fore.CYAN + f"\n resource IDs:", [resource["id"] for resource in self.resources if "id" in resource])
    ...
    async def run(self):
        while True:
            try:
                ...
                user_input = await self.session.prompt_async([('class:prompt', '\nYou: ')])
                # The color for the prompt is set by the style dictionary in PromptSession.
                # For example, "prompt": "#aaaaaa" sets the prompt text to a light gray color.
